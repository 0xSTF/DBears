...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // TODO(proj2): implement
        // TODO(proj4_part2): B+ tree locking
        // duplicate key case is handled by LeafNode.put()
        //


        // if root doesn't split, return
        // this put call goes to InnerNode.put() or LeafNode.put(), traverses down then up
        Optional<Pair<DataBox, Long>> result = root.put(key, rid);
        // if root splits, we need a new root
        if (result.isPresent()) {
            Pair<DataBox, Long> p = result.get();
            ...
            BPlusNode newRoot = new InnerNode(metadata, bufferManager, k, children, lockContext);
            updateRoot(newRoot);
        }
        // TODO(proj4_part3): B+ tree locking
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        /**Optional<Pair<DataBox, Long>> result = root.bulkLoad(data, fillFactor);
         ...
         DataBox new_root_key = pair.getFirst();

         List<DataBox> keys = new ArrayList<>();
         List<Long> children = new ArrayList<>();

         keys.add(new_root_key);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private BPlusTreeIterator(LeafNode node) {
            n = node;
            iter = n.scanAll();
        }

        private BPlusTreeIterator(LeafNode node, DataBox key) {
            n = node;
            iter = n.scanGreaterEqual(key);
        }
        ...
        public boolean hasNext() {
            ...
            while (true) {
                Optional<LeafNode> rightSib = n.getRightSibling();
